...
from model import get_model, get_model_onehot, model5
import numpy as np
...
train = np.load(Data_dir + '%s_train_onehot.npz' % name)
...
for i,(tra, val) in enumerate(kfold.split(X_en_tra, y_tra)):
    ...
    filepath = 'D:/pycharm_pro/gan_enhancer/second/model/our_model_onehot/%sModel%d.tf' % (name,i)
    ...
    history=model.fit(X_en_tra[tra], y_tra[tra], validation_data=(X_en_tes, y_tes), epochs=90, batch_size=batch_size,
